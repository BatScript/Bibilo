...
mongoose.connect(
  "mongodb+srv://bibilo:" +
    ...
    "@cluster0.9mefx.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
);
...
// Taken from passport-local-mongoose & passport-local npm page
    ...
    userData.findOne(
      ...
      async function (err, BookId) {
        ...
        for (const data of idArray) {
          const foundBookData = await productData
            .findOne({
              _id: data.name,
            })
            .catch(console.error);
  ...
  userData.findOne(
    ...
    (err, changeval) => {
      if (err) {
        ...
        if (change == "del") {
          changeval.cartItems.splice(
            changeval.cartItems.findIndex((val) => val.name === id),
            1
          );
          changeval.save();
        }
